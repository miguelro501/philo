...
/*   Updated: 2023/03/30 13:14:19 by miguelro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	think(t_philo *self)
{
	...
	if (!self->thinking)
	{
		...
		kill(self);
		if (handler()->n_threads % 2)
		{
			if (handler()->time_to_eat > handler()->time_to_sleep)
				ft_sleep(handler()->time_to_eat - handler()->time_to_sleep
					+ 10);
			else
				ft_sleep(handler()->time_to_sleep - handler()->time_to_eat
					+ 10);
		}
	}
}
